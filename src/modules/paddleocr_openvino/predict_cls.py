...
class TextClassifier(PredictBase):
    def __init__(self, args):
        ...
        self.core = ov.Core()
        self.cls_model = self.core.read_model(model=args.cls_model_dir)
        self.cls_compiled_model = self.core.compile_model(model=self.cls_model, device_name='CPU', 
            config={
            "PERFORMANCE_HINT": "LATENCY",   # minimize latency
            "INFERENCE_PRECISION_HINT": "f16",  # prefer FP16 compute
            "NUM_STREAMS": "1"   # avoid parallel streams, best for single request low latency
        })

        # Get input and output nodes for text detection.
        self.cls_input_layer = self.cls_compiled_model.input(0)
        self.cls_output_layer = self.cls_compiled_model.output(0)
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            outputs = self.cls_compiled_model({self.cls_input_layer: norm_img_batch})[self.cls_output_layer]
