...
class TextRecognizer(PredictBase):
    def __init__(self, args):
        ...
        self.core = ov.Core()
        self.rec_model = self.core.read_model(model=args.rec_model_file, weights=args.rec_model_weights)
        # If any input is 4-D (N,C,H,W) allow dynamic width by setting last dim to -1
        try:
            reshape_map = {}
            for input_layer in self.rec_model.inputs:
                ps = input_layer.partial_shape
                if len(ps) >= 4:
                    ps[3] = -1
                    reshape_map[input_layer] = ps
            if reshape_map:
                self.rec_model.reshape(reshape_map)
        except Exception:
            # keep original shapes if reshape fails
            pass

        self.rec_compiled_model = self.core.compile_model(model=self.rec_model, device_name='CPU', 
            config={
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            # Prefer explicit input mapping (handles models with multiple inputs).
            # Fallback to positional call for older OV versions.
            try:
                infer_res = self.rec_compiled_model({self.rec_input_layer: norm_img_batch})[self.rec_output_layer]
            except Exception:
                infer_res = self.rec_compiled_model([norm_img_batch])[self.rec_output_layer]

            # Ensure (N, T, C) for CTC postprocess: adapt common 2-D -> (N,1,C)
            if infer_res.ndim == 2:
                infer_res = np.expand_dims(infer_res, axis=1)

            rec_result = self.postprocess_op(infer_res)
            for rno in range(len(rec_result)):
                rec_res[indices[beg_img_no + rno]] = rec_result[rno]

        # make sure to return recognition results
        return rec_res
